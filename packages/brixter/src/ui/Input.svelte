<script lang="ts">
	...
	let { label, id, name, class: className, ...rest }: HTMLInputAttributes & { label?: string } = $props();
...
<div>
{#if label}
	<label for={inputId} class="block text-sm font-medium text-label mb-1">{label}</label>
{/if}
<input
	id={inputId}
	{name}
