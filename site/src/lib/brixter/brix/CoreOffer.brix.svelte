<script module lang="ts">
	export const brikDescription = 'Core offer presentation with key points and CTA.';

	export const brikFields = {
		features: {
			label: 'Offer points',
			itemLabel: 'Point',
			summaryField: 'title',
			item: {
				fields: {
					icon: {
						kind: 'icon',
						default:
							'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check"><path d="M20 6 9 17l-5-5"/></svg>'
					}
				}
			}
		},
		cta: {
			fields: {
				href: {
					default: '/admin'
				}
			}
		}
	};
</script>

<section class="bg-white px-6 py-20 dark:bg-gray-900">
	<div class="mx-auto grid max-w-5xl gap-10 md:grid-cols-[0.9fr_1.1fr] md:items-start">
		<div>
			<p
				class="text-muted mb-3 text-[11px] font-semibold tracking-wide uppercase"
				data-brixter-field="eyebrow"
			>
				Eyebrow
			</p>

			<h2
				class="font-display text-heading text-3xl md:text-4xl"
				data-brixter-field="headline"
				data-brixter-kind="richtext-inline"
			>
				Headline goes here.
			</h2>

			<p
				class="text-secondary mt-5 text-base leading-7"
				data-brixter-field="description"
				data-brixter-kind="richtext-inline"
			>
				A supporting description that follows the headline.
			</p>

			<a href="#" class="btn-site-primary mt-8" data-brixter-field="cta.label" data-brixter-bind="href:cta.href">Get started</a>
		</div>

		<div class="border border-gray-300 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
			<div
				class="border-b border-gray-300 p-6 last:border-b-0 dark:border-gray-700"
				data-brixter-collection-item="features"
			>
				<div class="flex items-start gap-3">
					<span
						class="mt-1 h-5 w-5 shrink-0 text-yellow-500 dark:text-yellow-400"
						data-brixter-field="features[].icon"
						data-brixter-kind="icon"
					>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check"><path d="M20 6 9 17l-5-5"/></svg>
					</span>
					<div>
						<h3 class="text-heading text-lg font-semibold" data-brixter-field="features[].title">
							Feature title
						</h3>
						<p
							class="text-secondary mt-2 text-sm leading-6"
							data-brixter-field="features[].text"
							data-brixter-kind="richtext-inline"
						>
							Feature description goes here.
						</p>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>